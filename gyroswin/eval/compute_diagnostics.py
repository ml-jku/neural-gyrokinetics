...
def dump_df_phi_diagnostics(
    ...
    phi_fft = np.moveaxis(phi_fft, 0, -1).copy()
    phi_fft = phi_fft.view(dtype=np.complex64).squeeze()
...
def main(args):
    ...
    with ThreadPoolExecutor(args.num_workers) as executor:
        ...
        returns = executor.map(extractor_fn, k_dirs)

    pred_df = []
    pred_poten = []
    for knth, phi in returns:
        pred_df.append(knth)
        pred_poten.append(phi)

    # average over predicted df and phi
    pred_poten = np.mean(pred_poten, axis=0)
    pred_df = np.mean(pred_df, axis=0)
    if compute_for_raw:
        dir = os.path.join(src_root, sim, "python_diagnostics")
    else:
