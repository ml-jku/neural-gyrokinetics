...
from neugk.utils import load_model_and_config
...
data = CycloneDataset(
    ...
    normalization=cfg.dataset.normalization,
    normalization_scope=cfg.dataset.normalization_scope,
    normalization_stats=traindata.stats,
    spatial_ifft=cfg.dataset.spatial_ifft,
    bundle_seq_length=cfg.model.bundle_seq_length,
...
assert traindata.stats == data.stats, "Normalization stats mismatch"
if cfg.dataset.offset > 0:
    ...
    with open(f"{parser.ckpt}/normalization_stats.pkl", "wb") as out:
        pickle.dump(traindata.stats, out)
...
if "ood" in parser.eval_sim:
    ...
    params = {}
    with h5py.File(
        f"/restricteddata/ukaea/gyrokinetics/preprocessed/{parser.eval_sim}"
    ) as infile:
    ...
    timesteps = traindata.get_timesteps(
        torch.tensor([0], dtype=torch.long), offset=cfg.dataset.offset
    )
    ...
    inputs = {
        k: getattr(sample, k).unsqueeze(0).to(device, non_blocking=True)
...
if parser.predict_on_different:
    ...
    conds = {
        k: torch.tensor([new_params[k]]).to(device, non_blocking=True)
        for k in new_params.keys()
    }

if "ood" not in parser.eval_sim:
    ...
    if IDX_END > len(data) - 2:
        ...
        timesteps = torch.cat(
            [
                timesteps,
                torch.arange(timesteps[:, -1].item() + delta, IDX_END, delta).unsqueeze(
                    0
                ),
            ],
            dim=1,
...
for key in input_fields:
    if cfg.dataset.normalization == "zscore":
        shift = (
            torch.tensor(traindata.stats[key]["full"]["mean"]).unsqueeze(0).to(device)
        )
        scale = (
            torch.tensor(traindata.stats[key]["full"]["std"]).unsqueeze(0).to(device)
        )
    elif cfg.dataset.normalization == "minmax":
        x_min = (
            torch.tensor(traindata.stats[key]["full"]["min"]).unsqueeze(0).to(device)
        )
        x_max = (
            torch.tensor(traindata.stats[key]["full"]["max"]).unsqueeze(0).to(device)
        )
...
with torch.no_grad():
    for idx in range(IDX_0, IDX_END + 1):
        ...
        if "ood" not in parser.eval_sim:
            ...
            sample = data[idx]
            gts_t = {
                k: getattr(sample, f"y_{k}").unsqueeze(0).to(device, non_blocking=True)
        ...
        for key in outputs.keys():
            ...
            if OUT_DIR:
                ...
                os.system(f"cp {raw_path}/input.dat {dirtarget}")
