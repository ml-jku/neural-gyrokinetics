...
def plot_flux_comp(gt_fluxes, pred_fluxes, dump_dir, gkw=False):
    ...
    ax.plot(
        np.array(list(gt_fluxes.keys())),
        list(gt_fluxes.values()),
        label="GT",
        c="tab:blue",
    )
    ax.plot(
...
def correlation_plot(model_corr, gt_corr, dump_dir):
    for key in model_corr.keys():
        ...
        gt_c = gt_corr[key]
        plt.plot(
            [val.cpu().item() for val in mc.keys()],
            [val.cpu().item() for val in mc.values()],
            c="tab:blue",
            label="Pred",
        )
        plt.plot(
            [val.cpu().item() for val in gt_c.keys()],
            [val.cpu().item() for val in gt_c.values()],
            c="tab:red",
            label="Identity",
        )
        plt.legend()
...
if "iteration" in DUMP_DIR:
    ...
    sims_to_eval = [
        dir
        for dir in os.listdir(DUMP_DIR)
        if not dir.endswith("Lin") and "alpha" not in dir
    ]
...
for sim in sims_to_eval:
    ...
    if GT_FLUX_DIR is None or (
        "param_scans" in args.pred_path and "iteration" not in args.pred_path
    ):
    ...
    gt_fluxes = np.loadtxt(
        os.path.join("_".join(GT_FLUX_DIR.split("_")[:2]), "fluxes.dat")
    )
    ...
    if args.gkw:
        ...
        if os.path.isfile(os.path.join(DUMP_DIR, k_dir[0], "flux")):
            ...
            for k_file in k_dir[:-2]:
                ...
                if k_file.startswith("K"):
                    ...
                    if not os.path.isfile(
                        os.path.join("_".join(GT_DIR.split("_")[:2]), gt_k_file_time)
                    ):
                ...
                with open(
                    f"{'_'.join(GT_DIR.split('_')[:2])}/{gt_k_file_time}.dat", "r"
                ) as file:
                    for line in file:
                        line_split = line.split("=")
                        if line_split[0].strip() == "TIME":
            ...
            for k_file in tqdm(
                k_files, total=len(k_files), desc="Computing integral..."
            ):
                assert os.path.exists(
                    os.path.join(DUMP_DIR, k_file, "FDS")
                ), f"FDS file for {k_file} missing..."
