...
def _ae_df_stats(ckpt_dir, data_path):
    ...
    key = (os.path.realpath(ckpt_dir), os.path.realpath(str(data_path)) if data_path else None)
    if key in _AE_STATS_CACHE:
        return _AE_STATS_CACHE[key]
    train_cfg = _notebook_safe(
...
def _decoder_cond_keys(train_cfg):
    ...
    model_cfg = train_cfg.autoencoder if "autoencoder" in train_cfg else train_cfg.model
...
def _vae_latent_stats(ckpt_dir, device):
    ...
    if os.path.exists(old):
        ...
        return (
            torch.tensor(np.mean(arr, axis=0), device=device),
    ...
    *,
    ...
    try:
        for fi, entry in tqdm(list(real_by_fi.items()), desc="  vae sample"):
            ...
            cond = get_conditioning(cond_keys, meta, device) if cond_keys else None
            dfs = []
            remaining = int(n_per_traj)
            while remaining > 0:
                bs = min(batch_size, remaining)
                gen = sample_vae_prior(
                    model, bs, cond, pad_axes, device, mean=z_mean, std=z_std
                ).float()
                for b in range(bs):
...
def sample_vqvae(
    ...
    *,
    ...
    cond_keys = _decoder_cond_keys(train_cfg)
    vq_prior = (
        compute_codebook_prior(vq_index_pkl, model.vq.codebook_size) if vq_index_pkl else None
    )
    valset = ref_runner.valsets[0]
    ...
    try:
        for fi, entry in tqdm(list(real_by_fi.items()), desc="  vqvae sample"):
            ...
            while remaining > 0:
                bs = min(batch_size, remaining)
                gen = sample_vqvae_random(model, bs, cond, pad_axes, device, prior=vq_prior).float()
                for b in range(bs):
                    dfs.append(_denormalize(gen[b].cpu(), df_mean, df_std))
                remaining -= bs
            out[fi] = {"df": dfs, "label": entry["label"]}
    ...
    *,
    ...
    try:
        for fi, entry in tqdm(list(real_by_fi.items()), desc="  ar sample"):
            ...
            while remaining > 0:
                bs = min(batch_size, remaining)
                gen = _ar_sample_decode(ar_model, ae_model, ar_cfg, cond, bs, device).float()
                for b in range(bs):
                    dfs.append(_denormalize(gen[b].cpu(), df_mean, df_std))
                remaining -= bs
            out[fi] = {"df": dfs, "label": entry["label"]}
...
@torch.no_grad()
def sample_diff_inplace(
    ref_runner, real_by_fi, *, n_per_traj=32, n_denoising_steps=15, batch_size=32
):
    ...
    *,
    ...
    for fi, entry in real_by_fi.items():
        ...
        if nn_train_fi is None:
            print(
                f"  [nn] no training trajectory with dist > {min_dist} for "
        ...
        sat_flat = [flat for flat, t in flat_by_file.get(nn_train_fi, []) if t >= sat_t_min]
        if not sat_flat:
            print(
                f"  [nn] no saturated frames for train file {nn_train_fi}; "
        ...
        if verbose:
            extra = (
                f" (skipped {n_skipped} dist<={min_dist} match{'es' if n_skipped != 1 else ''})"
                if n_skipped
                else ""
            )
            print(
                f"  [nn] eval fi={fi} -> train fi={nn_train_fi} "
...
def split_half_fid(real_feats, gen_full_feats, *, k, n_components=64, seed=42):
    ...
    if n_full < 2 * k:
        raise ValueError(f"split_half_fid: need >=2K={2*k} gen samples, got {n_full}")
    rng = np.random.default_rng(seed)
