        ...
        try:
            ...
            while True:
                ...
                if "Energy flux" in output:
                    flux = float(output.strip().replace(" ", "").replace("Energyflux:", ""))
                    print(f"\tflux: {flux}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run GKW simulation and extract energy flux values.")
    parser.add_argument("dump_dir", type=str, help="Path to the directory containing simulation data.", default=DUMP_DIR)
