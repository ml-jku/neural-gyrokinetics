...
def plot_4x4_2D_raspec(x1, x2, **kwargs):
    _ = kwargs
    labels = [r"v_{par}", r"v_{\mu}", r"s", r"k_x", r"k_y"]
    ...
    for i, j in comb:
        ...
        xx = x1[0].mean(other)
        yy = x2[0].mean(other)
        xx_raspec = radially_averaged_power_spectrum_nd(xx)
        yy_raspec = radially_averaged_power_spectrum_nd(yy)
        ax[i, j].loglog(xx_raspec, label="Pred spec", c="r", lw=3)
        ax[i, j].loglog(yy_raspec, label="GT spec", c="k", lw=3)
        ax[i, j].grid(True)
...
def power_spectrum_2d(image, axes):
    """
    Compute the 2D power spectrum of a complex input along the specified axes.
    """
    # Combine real and imaginary parts into a complex array
    image_complex = image[0] + 1j * image[1]
    # Subtract the mean to remove DC component
    image_complex = image_complex - image_complex.mean()
    # Compute FFT along the specified axes
    fourier_transform = np.fft.fftn(image_complex, axes=axes)
    fourier_transform_shifted = np.fft.fftshift(fourier_transform, axes=axes)
    power_spectrum = np.abs(fourier_transform_shifted) ** 2
    return power_spectrum


def plot4x4_sided_spec(x1, x2, title="", mark_bad=False, average=True):
    labels = [r"v_{par}", r"v_{\mu}", r"s", r"k_x", r"k_y"]
    comb = torch.combinations(torch.arange(5), 2).tolist()
    ...
    fig.suptitle(title)
    c_map = plt.colormaps["plasma"]
    c_map.set_bad("k")

    for i, j in comb:
        other = tuple([o for o in range(5) if o != i and o != j])

        x1_spec = power_spectrum_2d(x1, axes=(i, j))
        x2_spec = power_spectrum_2d(x2, axes=(i, j))

        # Average along the remaining axes
        x1_plot = x1_spec.mean(axis=other)
        x2_plot = x2_spec.mean(axis=other)

        if mark_bad:
            x1_std = x1_spec.std(axis=other)
            x2_std = x2_spec.std(axis=other)
            x1_plot[x1_std == 0] = np.nan
            x2_plot[x2_std == 0] = np.nan

        x1_plot = np.log(x1_plot + 1e-10)
        x2_plot = np.log(x2_plot + 1e-10)
...
def generate_val_plots(x_rollout, y, ts, phase):
    plots = {}
    val_plots_dict = {
