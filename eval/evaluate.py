...
def evaluate(
    rank, world_size, model, loss_wrap, valsets, valloaders, opt, epoch, cfg, device, loss_val_min
):
    # Validation loop
    ...
    if (epoch % val_freq) == 0 or epoch == 1:
        ...
        for val_idx, (valset, valloader) in enumerate(zip(valsets, valloaders)):
            ...
            # TODO loss_wrap in metrics (flux integrals)
            metric_fn_list = {
            ...
            for phase in ["linear", "saturated"]:
                for key in cfg.model.losses:
                    metrics[phase][key] = torch.zeros(
                        [len(metric_fn_list), tot_eval_steps]
                    )
                    ...
                    for i in range(len(inputs_list)):
                        ...
                        if denormalize:
                            ...
                            for key in input_fields:
                                ...
                                gts[key] = torch.stack(
                                    [
                                        valset.denormalize(f, *{key: gts[key][b]})
                                        for b, f in enumerate(
                                            idx_data["file_index"].tolist()
                                        )
                                    ]
            ...
            for phase in metrics.keys():
                for out_type in metrics[phase].keys():
                    metrics[phase][out_type] = metrics[phase][
                        out_type
                    ] / n_timesteps_acc[phase].unsqueeze(0)
