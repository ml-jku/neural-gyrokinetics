    ...
    if cfg.mode == "train":
        ...
        for epoch in range(n_epochs):
            ...
            if epoch % cfg.validation.validate_every_n_epochs == 0:
                ...
                metric_fn_list = {
                    "relative_norm_mse": partial(relative_norm_mse, dim_to_keep=0),  # to average across all dimensions except timesteps
                ...
                with torch.no_grad():
                    for idx, sample in enumerate(valloader):
                        ...
                        if idx == 0:
                            ...
                            val_plots[f"GT vs Pred at time {ts[0].item():.2f} (Linear Phase)"] = plot4x4_sided(
                                y[0, ...].to("cpu"), x_rollout[0, 0, ...]  # first timestep and batch
                            )
                            val_plots[f"Pred at time {ts[0].item():.2f} (Linear Phase)"] = distribution_5D(
                            ...
                            val_plots[f"GT vs Pred at time {ts[0].item():.2f} (Saturated Phase)"] = plot4x4_sided(
                                y[0, ...].to("cpu"), x_rollout[0, 0, ...]  # first timestep and batch
                            )
                            val_plots[f"Pred at time {ts[0].item():.2f} (Saturated Phase)"] = distribution_5D(
                                x_rollout[0, 0, ...]  # first timestep and batch
                            )
                        else:
                            # metrics tensor will have shape [number_of_metrics, n_timesteps]
